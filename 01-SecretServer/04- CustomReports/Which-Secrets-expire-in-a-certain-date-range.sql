...
WHERE
	...
	AND
		(
			tbSecret.IsCustomExpiration = 0 AND tbSecret.ExpiredFieldChangedDate + tbSecretType.ExpirationDays >= #STARTDATE
				AND tbSecret.ExpiredFieldChangedDate + tbSecretType.ExpirationDays < #ENDDATE  
			OR
			tbSecret.IsCustomExpiration = 1 AND tbSecret.ExpiredFieldChangedDate + tbSecret.CustomExpirationDays >= #STARTDATE
